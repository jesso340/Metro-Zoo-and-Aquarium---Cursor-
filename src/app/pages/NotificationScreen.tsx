...
type NotificationLocationState = {
  showFirstNotificationOnly?: boolean;
  showSecondNotificationOnly?: boolean;
};

export function NotificationScreen() {
  const navigate = useNavigate();
  const location = useLocation();
  const st = location.state as NotificationLocationState | null;
  const hideFirstNotification = Boolean(st?.showSecondNotificationOnly);
  const hideSecondNotification = Boolean(st?.showFirstNotificationOnly);
  ...
  const handleOpenAssistant = () => {
    navigate("/app", { state: { openZooAssistant: true } });
  };
  ...
  return (
    <div className="min-h-screen bg-[#000000] flex flex-col max-w-[430px] mx-auto relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center px-6 relative z-10 pt-20">
        ...
        {/* Notifications (stacked: newest on top) */}
        <div className="w-full flex flex-col gap-3">
          {!hideFirstNotification && (
            ...
            >
              <IOSNotification
                time="now"
                title="You're here — welcome in, Grace!"
                body="We noticed you've arrived. Your Metro Zoo Guide Assistant is ready — tap to open the app and start planning with the chatbot."
                expanded={true}
